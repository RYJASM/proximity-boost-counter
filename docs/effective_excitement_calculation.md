...
| Scenery delta | Exact for above-ground stations. For underground stations the game returns a fixed item count of `40` (capped at 47, score = 40 × 5 = 200) — this value itself is exact. The only approximation risk is in *detecting* underground: the plugin compares the surface tile's base height to the station's base Z, which may differ slightly on sloped terrain where `TileElementHeight()` (used by the game) accounts for slope interpolation that the API doesn't directly expose. |
